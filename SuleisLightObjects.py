...
class MainDatabase:  # Class that represents main database of users, strips, sections, and active patterns
    def __init__(self):
        self.users_database = pd.DataFrame(
            columns=["User ID", "User Object"])  # DataFrame of users for fast user locating
        self.strips_database = pd.DataFrame(columns=["Strip ID",
                                                     "Strip Object"])  # use to make sure one strip isn't attached to multiple accounts, and fast strip object locating

    def return_user_object(self,
                           user):  # returns the user's object if user is returning, or makes a new one and then returns it if it is a new user. If user is none (no logged in?), returns false
        if user != None:
            ...
            try:
                ...
                self.users_database = self.users_database.append(
                    pd.Series({"User ID": new_user.user_id, "User Object": new_user}, name=user.id))
                return new_user
        else:
            return False

    def check_and_add_strip(self, strip_id, leds_count,
                            display_name):  # if strip already exists, returns false, if strip is new, adds it to list and return the strip object
        try:
            strip_obj = self.strips_database.loc[strip_id][
                "Strip Object"]  # Returns error if strip not already in database
            ...
            self.strips_database = self.strips_database.append(
                pd.Series({"Strip ID": strip_id, "Strip Object": new_strip}, name=strip_id))
            return new_strip
...
class User:  # Class to represent a user. Can aggregate multiple strips
    ...
    def add_strip(self, strip_id, leds_count, display_name):  # add new strip to user with given parameters
        ...
        if display_name in self.light_strips.keys():
            ...
            strip_object = self.main_database.check_and_add_strip(strip_id, leds_count,
                                                                  display_name)  # returns False if strip already registered, or the new strip object if not
...
class Strip:  # Class representing a Strip. Can aggregate multiple sections.
    def __init__(self, strip_id, leds_count, display_name):
        ...
        self.default_section = Section(self, 0, leds_count - 1,
                                       str(uuid.uuid4()))  # Make aa default section for this strip
        self.sections_list = [
            self.default_section]  # List of all active sections in this strip. Inactive sections (those covered fully by new section(s)) will be removed.
    ...
    def add_new_section_and_update(self, start_led,
                                   end_led):  # Add new section to the strip (and test that the range is valid
        if start_led < 0:
            ...
            if leds_and_sections_new[led_num] not in sections_found:
                ...
                if not leds_and_sections_new[led_num] == sections_found[-1]:
                    ...
                    for led_num2 in range(led_num + 1, self.leds_count):
                        if leds_and_sections_new[led_num2] == leds_and_sections_new[led_num]:
    ...
    def add_alarm(self, minutes_from_midnight):  # add an alarm to the list
        self.alarms[minutes_from_midnight] = {"repeat": False, "status": {"mode": "wf", "color": {"r": 255, "g": 255,
                                                                                                  "b": 255}}}  # wf for white flash

    def return_sections_and_patterns_dict(
            self):  # Returns a dict of each section's Pattern (not light status). We can use this for configuring the strip sections.
        ...
        for led_section in self.leds_and_sections:
            if curr_range_pattern == led_section.current_mode:
                ...
                ranges.append(
                    {"range_start": curr_range_start, "range_end": curr_range_end, "range_pattern": curr_range_pattern,
                     "range_section": curr_range_section})
    ...
    def return_light_status_as_json(
            self):  # Returns an * delimited list of JSON strings representing each Section (and associated light status) in the list.
...
class Section:  # Class to represent a Section of lights. Has a Pattern.
    def __init__(self, strip, initial_start_led, initial_end_led, display_name):
        ...
        self.set_pattern(SolidPattern({'r': 255, 'g': 255, 'b': 255},
                                      str(uuid.uuid4())))  # Set the default pattern to a solid 'warm' light color
    ...
    def update_light_status(
            self):  # Calls the appropriate update function for the associated pattern, and stores the current light status
...
class TimePattern:
    ...
    def get_current_status_dict(self):
        ...
        minutes_since_midnight = round(
            (now - now.replace(hour=0, minute=0, second=0, microsecond=0)).total_seconds() / 60)
        if self.gradient_type == "fade":
            self.color_dict = self.time_color_table.append(
                pd.Series({'r': np.NaN, 'g': np.NaN, 'b': np.NaN}, name=minutes_since_midnight)).interpolate(
                method='index').round().loc[minutes_since_midnight].to_dict()
...
class SunPattern:
    ...
    def get_current_status_dict(self):
        ...
        print(self.lat)
        print(self.long)
        minutes_since_midnight_dawn = round(
            (loc.dawn() - now.replace(hour=0, minute=0, second=0, microsecond=0)).total_seconds() / 60)

        minutes_since_midnight_sunrise = round(
            (loc.sunrise() - now.replace(hour=0, minute=0, second=0, microsecond=0)).total_seconds() / 60)
        ...
        self.time_color_table = pd.DataFrame.from_dict(
            {0: self.sun_down_color_dict, minutes_since_midnight_dawn: self.sun_down_color_dict,
             minutes_since_midnight_sunrise: self.sun_up_color_dict,
             minutes_since_midnight_sunset: self.sun_up_color_dict,
             minutes_since_midnight_dusk: self.sun_down_color_dict}, orient='index')
        ...
        current_color = self.time_color_table.append(
            pd.Series({'r': np.NaN, 'g': np.NaN, 'b': np.NaN}, name=minutes_since_midnight_now)).interpolate(
            method='index').round().loc[minutes_since_midnight_now].to_dict()
